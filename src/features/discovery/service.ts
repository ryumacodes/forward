...
import { localDemoMode } from '../local-demo/config'
import { importLocalDiscovery, saveLocalDiscoveries, verifyLocalSupplier } from '../local-demo/store'
...
export async function discoverSuppliers(input:{product:string;location:string;profileId:DiscoveryProfileId;organizationId?:string;requiresHalal?:boolean}) {
  if(localDemoMode){
    const response=await fetch('/api/local-demo/discover',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(input)})
    const data=await response.json().catch(()=>({})) as {suppliers?:DiscoveredSupplier[];error?:string}
    if(!response.ok)throw new Error(data.error||'The local live scraper could not retrieve supplier pages.')
    if(!Array.isArray(data.suppliers)||!data.suppliers.length)throw new Error('No relevant public supplier pages were retrievable. Try a broader product description.')
    await saveLocalDiscoveries(data.suppliers)
...
export async function importDiscoveredSupplier(input:{organizationId:string;evidenceId:string}) {
  if(localDemoMode)return await importLocalDiscovery(input.evidenceId)
...
export async function screenDiscoveredSuppliers(input:{candidates:Pick<DiscoveredSupplier,'id'>[];organizationId:string;onProgress?:(message:string)=>void;onImported?:(count:number)=>void|Promise<void>;operations?:ScreeningOperations}):Promise<SupplierScreeningSummary>{
  ...
  const verifyAbn=input.operations?.verifyAbn??(localDemoMode?verifyLocalSupplier:verifySupplierAbn)
