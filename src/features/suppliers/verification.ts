...
export function prepareSupplier(name: string, abn: string, phone: string, existing: ImportedSupplier[], email=''): ImportedSupplier {
  ...
  return {id: crypto.randomUUID(), name:name.trim(), abn:normalisedAbn, phone:phone.trim(),email:email.trim()||undefined, importedAt:new Date().toISOString(), status:'Awaiting registry check', authorised:false}
}
