...
import { X, ShieldCheck, ArrowRight, Check, AlertCircle } from 'lucide-react'
...
export function NewRecovery({ onClose, onCreate }: { onClose: () => void; onCreate: (request: Recovery) => void | Promise<void> }) {
  ...
  return <dialog className="voice-dialog" aria-labelledby="voice-dialog-title" ref={dialog} onCancel={onClose} onClick={e => { if (e.target === e.currentTarget) onClose() }}><form onSubmit={async e => { e.preventDefault(); if(saving)return; setSaving(true);setSaveError(''); const data = new FormData(e.currentTarget); try { await onCreate({ id: `REC-${Date.now().toString().slice(-6)}`, item: String(data.get('item')).trim(), quantity: Number(data.get('quantity')), unit: String(data.get('unit')), budget: Number(data.get('budget')), deadline: String(data.get('deadline')), location: String(data.get('location')).trim(), status: 'Ready to source', category: 'Voice request', brief, purchaseMode: String(data.get('purchaseMode')), confirmationChannel: String(data.get('confirmationChannel')), minimumPaymentDays: Number(data.get('minimumPaymentDays')), maximumDepositPercent: Number(data.get('maximumDepositPercent')), requiresHalal:data.get('halal')==='yes', cut:String(data.get('cut')||'')||undefined, freshness:(String(data.get('freshness')||'')||undefined) as Recovery['freshness'] }) } catch(error) {setSaveError(error instanceof Error ? error.message : 'Could not save request. Try again.')} finally {setSaving(false)} }}>
    <div className="modal-head"><div><span className="eyebrow">YOUR VOICE PROCUREMENT AGENT</span><h2 id="voice-dialog-title">What do you need?</h2></div><button type="button" className="icon-button" aria-label="Close" onClick={onClose}><X size={20}/></button></div>
    ...
    <label className="brief-label">Your request<textarea value={brief} onChange={e => {setBrief(e.target.value);setNormalization(null)}} placeholder="Or type what you need…" rows={3}/></label>
    ...
    <label>When a quote meets every requirement<select name="purchaseMode"><option value="confirm">Check with me before issuing a purchase order</option></select></label>
    ...
    {details && <>
      ...
      <button className="primary full request-submit" type="submit" disabled={saving}>{saving ? 'Saving…' : 'Confirm & create recovery'} <ArrowRight size={16}/></button>
    </>}
  </form></dialog>
}
