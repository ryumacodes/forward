...
export function NewRecovery({ onClose, onCreate }: { onClose: () => void; onCreate: (request: Recovery) => void | Promise<void> }) {
  ...
  const [draft,setDraft] = useState({item:'',quantity:'',unit:'kg',budget:'',deadline:'',location:'',minimumPaymentDays:'14',maximumDepositPercent:'0'})
  ...
  return <dialog className="voice-dialog" aria-labelledby="voice-dialog-title" ref={dialog} onCancel={onClose} onClick={e => { if (e.target === e.currentTarget) onClose() }}><form onSubmit={async e => { e.preventDefault(); if(saving)return; setSaving(true);setSaveError(''); const data = new FormData(e.currentTarget); try { await onCreate({ id: `REC-${Date.now().toString().slice(-6)}`, item: String(data.get('item')).trim(), quantity: Number(data.get('quantity')), unit: String(data.get('unit')), budget: Number(data.get('budget')), deadline: String(data.get('deadline')), location: String(data.get('location')).trim(), status: 'Ready to source', category: 'Voice request', brief, purchaseMode: String(data.get('purchaseMode')), confirmationChannel: String(data.get('confirmationChannel')), minimumPaymentDays: Number(data.get('minimumPaymentDays')), maximumDepositPercent: Number(data.get('maximumDepositPercent')) }) } catch(error) {setSaveError(error instanceof Error ? error.message : 'Could not save request. Try again.')} finally {setSaving(false)} }}>
