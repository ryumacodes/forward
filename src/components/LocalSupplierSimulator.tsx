...
import { getLocalCallSession, saveLocalCallSession, subscribeLocalCalls, type LocalCallSession, type LocalSupplierQuote } from '../features/local-demo/store'
...
export function LocalSupplierSimulator(){
  ...
  const answer=()=>{const next={...session,status:'in_progress' as const,transcript:[...session.transcript]};agentTurn(next,`Hi, I’m Sarah, the SourcePilot AI procurement assistant calling for Flinders Kitchen about ${next.request.quantity} ${next.request.unit} of ${next.request.item}. Your business was found from its public supplier website. Is now a convenient time for a short quote enquiry? No order will be placed on this call.`);update(next)}
  const submit=()=>{
  ...
  return <main className="local-supplier-shell"><section className="local-supplier-card">
    <div className="local-demo-banner"><ShieldCheck size={17}/><span><strong>Local call simulator</strong>No phone network, provider API, SMS, order, or payment is used.</span></div>
    <div className="local-call-heading"><div><span className="eyebrow">INCOMING SOURCEPILOT CALL</span><h1>{session.supplier.name}</h1><p>{session.request.quantity} {session.request.unit} {session.request.item} · up to ${session.request.budget}</p></div><span className={`local-call-state ${session.status}`}>{session.status.replace('_',' ')}</span></div>
