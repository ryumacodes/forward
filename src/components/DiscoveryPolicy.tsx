...
export function DiscoveryPolicy({organizationId,request,onImported}:{organizationId?:string;request?:ProcurementRequest;onImported?:()=>void|Promise<void>}) {
  ...
  return <section className="policy-panel discovery-workbench" ref={panel} aria-label="Supplier discovery for selected request">
    <div className="section-header"><div><h2>Live supplier discovery</h2><p>{localDemoMode?'Retrieves public supplier pages in real time, captures evidence, then ranks locally without paid APIs.':'Searches public supplier pages, captures evidence, then uses embeddings to match product language.'}</p></div><span className="demo-badge">{localDemoMode?'Local live-scrape sandbox':'Evidence before eligibility'}</span></div>
    ...
    {error&&<div className="discovery-error" role="alert"><AlertCircle size={17}/><span><strong>Live search is not available yet.</strong>{error}<small>{localDemoMode?'Check your internet connection and try a broader product term. No cached result is presented as live.':'Deploy the discovery function, migration, and server-side OpenAI secret. No result below is being presented as live.'}</small></span></div>}
    ...
    <p className="voice-message">{localDemoMode?'Local mode fetches the cited public pages at search time and ranks by explainable term overlap. Registry checks and calls are simulated and visibly labelled; owner authorization remains mandatory.':'The model finds and structures candidates; text-embedding-3-small handles catalogue-language similarity. Neither can override ABR, owner authorization, certification, opt-out, or minimum product-match rules.'}</p>
  </section>
}
