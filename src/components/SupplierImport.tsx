...
export function SupplierImport({suppliers,onImport,onUpdate,activeRequestId,activeRequest,organizationId,onCallStarted,onQueueChanged}:SupplierImportProps) {
  ...
  async function queueSelected(){
    ...
    try{if(localDemoMode){if(!activeRequest)throw new Error('Select a request before starting the simulated calls.');const chosen=suppliers.filter(item=>selected.has(item.id));if(!chosen.length)throw new Error('Select at least one supplier.');const popups=chosen.map(()=>window.open('about:blank','_blank'));for(let index=0;index<chosen.length;index++){const session=await createLocalCallSession(activeRequest,chosen[index]);const url=`/?supplier-simulator=1&session=${encodeURIComponent(session.id)}`;if(popups[index])popups[index]!.location.href=url}setSelected(new Set());onCallStarted?.(`${chosen.length} simulated supplier calls are ringing. Respond in each supplier tab; quotes will stream back to the dashboard.`);return}const result=await enqueueSupplierCalls([...selected],activeRequestId||'');setSelected(new Set());onCallStarted?.(`${result.queued} supplier${result.queued===1?'':'s'} added to the calling queue.${result.conversationId?' The first call is starting.':''}`);await onQueueChanged?.()}
