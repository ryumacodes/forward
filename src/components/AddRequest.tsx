...
import { Check, ClipboardList, MapPin, ShieldCheck, X } from '../icons'
import type { ProcurementRequest } from '../features/requests/data'

type AddRequestProps = {
  requests: ProcurementRequest[]
  onAdd: (request: ProcurementRequest) => ProcurementRequest | Promise<ProcurementRequest>
  onClose: () => void
}
...
type Draft = {
  ...
  sourcingMode: 'first_qualifying' | 'compare'
  purchaseMode: 'confirm' | 'preauthorized'
...
function nextRequestId(requests: ProcurementRequest[]) {
  const highest = requests.reduce((value, request) => Math.max(value, Number(request.id.match(/\d+/)?.[0]) || 0), 0)
  return `REQ-${String(highest + 1).padStart(3, '0')}`
}

export function AddRequest({ requests, onAdd, onClose }: AddRequestProps) {
  const [step, setStep] = useState(0)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [draft, setDraft] = useState<Draft>({ item: '', category: '', quantity: '', unit: 'units', budget: '', deadline: defaultDeadline(), location: '24 Flinders Lane, Melbourne', minimumPaymentDays: '0', maximumDepositPercent: '0', sourcingMode: 'compare', purchaseMode: 'confirm', confirmationChannel: 'sms', requiresHalal: 'no', freshness: 'either', brief: '' })
  ...
  async function submit() {
    ...
    try {
      await onAdd({
        id: nextRequestId(requests), item: draft.item.trim(), category: draft.category.trim(), quantity: Number(draft.quantity), unit: draft.unit, budget: Number(draft.budget), deadline: draft.deadline, location: draft.location.trim(), status: 'Ready to source',
        minimumPaymentDays: Number(draft.minimumPaymentDays), maximumDepositPercent: Number(draft.maximumDepositPercent), sourcingMode: draft.sourcingMode, purchaseMode: draft.purchaseMode, confirmationChannel: draft.confirmationChannel,
  ...
  return <dialog className="request-wizard" aria-labelledby="request-wizard-title" ref={node => { if (node && !node.open) node.showModal() }} onCancel={onClose}>
    ...
    <form onSubmit={event => { event.preventDefault(); step < 2 ? continueFlow() : submit() }}>
      ...
      {step === 2 && <section className="wizard-panel"><div className="wizard-intro"><ShieldCheck size={20}/><div><h3>Set purchasing controls</h3><p>Choose whether Sarah stops at the first qualifying quote or compares suppliers for your decision.</p></div></div><div className="form-grid"><label><span className="field-label">Sourcing approach</span><select name="sourcingMode" value={draft.sourcingMode} onChange={event => {const sourcingMode=event.target.value as Draft['sourcingMode'];setDraft(current=>({...current,sourcingMode,purchaseMode:sourcingMode==='compare'?'confirm':current.purchaseMode}))}}><option value="compare">Compare suppliers, then I choose</option><option value="first_qualifying">Stop at first qualifying quote</option></select></label><label><span className="field-label">Minimum payment terms</span><select name="minimumPaymentDays" value={draft.minimumPaymentDays} onChange={event => update('minimumPaymentDays', event.target.value)}><option value="0">Due on delivery accepted</option><option value="7">Net 7 or better</option><option value="14">Net 14 or better</option><option value="30">Net 30 or better</option></select></label><label><span className="field-label">Maximum deposit</span><select name="maximumDepositPercent" value={draft.maximumDepositPercent} onChange={event => update('maximumDepositPercent', event.target.value)}><option value="0">No deposit</option><option value="10">Up to 10%</option><option value="25">Up to 25%</option><option value="50">Up to 50%</option></select></label><label><span className="field-label">Purchase approval</span><select name="purchaseMode" value={draft.purchaseMode} onChange={event => update('purchaseMode', event.target.value)}><option value="confirm">Owner confirms every order</option><option value="preauthorized" disabled={draft.sourcingMode==='compare'}>Pre-authorised within all limits</option></select></label><label><span className="field-label">Confirmation channel</span><select name="confirmationChannel" value={draft.confirmationChannel} onChange={event => update('confirmationChannel', event.target.value)}><option value="sms">Text message</option><option value="call">Call first, then text</option></select></label><label><span className="field-label">Halal certification</span><select name="requiresHalal" value={draft.requiresHalal} onChange={event => update('requiresHalal', event.target.value)}><option value="no">Not required</option><option value="yes">Required</option></select></label><label><span className="field-label">Freshness</span><select name="freshness" value={draft.freshness} onChange={event => update('freshness', event.target.value)}><option value="either">Fresh or frozen</option><option value="fresh">Fresh only</option><option value="frozen">Frozen only</option></select></label></div><div className="request-review"><div><span>Request</span><strong>{draft.quantity} {draft.unit} · {draft.item}</strong><small>{draft.category}</small></div><div><span>Delivery</span><strong>{new Date(draft.deadline).toLocaleString('en-AU')}</strong><small>{draft.location}</small></div><div><span>Budget</span><strong>{formattedBudget} maximum</strong><small>Fees must fit inside this total</small></div><div><span>Strategy</span><strong>{draft.sourcingMode === 'compare' ? 'Compare, then owner chooses' : 'First qualifying quote'}</strong><small>{draft.purchaseMode === 'confirm' ? 'Owner confirms before PO' : 'Pre-authorised within limits'}</small></div></div></section>}
