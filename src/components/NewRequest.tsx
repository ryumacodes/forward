...
export function NewRequest({ onClose, onCreate }: { onClose: () => void; onCreate: (request: ProcurementRequest, options?: {keepOpen?:boolean}) => void | Promise<void> }) {
  ...
  const [purchaseMode,setPurchaseMode] = useState<NonNullable<ProcurementRequest['purchaseMode']>>('preauthorized')
  ...
  async function reviewRequest() {
    ...
    try {
      ...
      const spokenSourcingMode = parseSourcingMode(brief)
      if (spokenSourcingMode) {
        setSourcingMode(spokenSourcingMode)
        if (spokenSourcingMode === 'compare') setPurchaseMode('confirm')
      }
  ...
  return <dialog className={`voice-dialog ${details ? 'review-mode' : 'conversation-mode'}`} aria-labelledby="voice-dialog-title" ref={dialog} onCancel={onClose} onClick={e => { if (e.target === e.currentTarget) onClose() }}><form className="voice-dialog-form" onSubmit={async e => { e.preventDefault(); if(saving)return; setSaving(true);setSaveError(''); const data = new FormData(e.currentTarget); try { await onCreate({ id: `REQ-${Date.now().toString().slice(-6)}`, item: String(data.get('item')).trim(), quantity: Number(data.get('quantity')), unit: String(data.get('unit')), budget: Number(data.get('budget')), deadline: String(data.get('deadline')), location: String(data.get('location')).trim(), status: 'Ready to source', category: 'Voice request', brief, sourcingMode: String(data.get('sourcingMode')) as ProcurementRequest['sourcingMode'], purchaseMode: String(data.get('purchaseMode')) as ProcurementRequest['purchaseMode'], buyingProfile: String(data.get('buyingProfile')) as ProcurementRequest['buyingProfile'], confirmationChannel: String(data.get('confirmationChannel')), minimumPaymentDays: Number(data.get('minimumPaymentDays')), maximumDepositPercent: Number(data.get('maximumDepositPercent')), requiresHalal:data.get('halal')==='yes', cut:String(data.get('cut')||'')||undefined, freshness:(String(data.get('freshness')||'')||undefined) as ProcurementRequest['freshness'] }) } catch(error) {setSaveError(error instanceof Error ? error.message : 'Could not save request. Try again.')} finally {setSaving(false)} }}>
    ...
    <label>Sourcing approach<select name="sourcingMode" value={sourcingMode} onChange={event=>{const mode=event.target.value as NonNullable<ProcurementRequest['sourcingMode']>;setSourcingMode(mode);if(mode==='compare')setPurchaseMode('confirm')}}><option value="first_qualifying">Stop at the first quote that meets every rule</option><option value="compare">Compare all selected suppliers, then let me choose</option></select></label>
    <label>When a quote meets every requirement<select name="purchaseMode" value={purchaseMode} onChange={event=>setPurchaseMode(event.target.value as NonNullable<ProcurementRequest['purchaseMode']>)}><option value="preauthorized" disabled={sourcingMode==='compare'}>Issue the PO automatically within these exact limits</option><option value="confirm">Check with me before issuing a purchase order</option></select></label>
    <label>How Sarah should send exceptions and the final result<select name="confirmationChannel"><option value="call">Call me; text me if calling is unavailable</option><option value="sms">Text me</option></select></label>
    ...
    {details && <>
      ...
      {sourcingMode==='compare'&&<p className="form-note"><ShieldCheck size={18}/><span>Sarah will finish the selected supplier queue, rank every complete quote, and wait for you to choose before issuing a purchase order.</span></p>}
      {sourcingMode==='first_qualifying'&&purchaseMode==='preauthorized'&&<p className="form-note"><ShieldCheck size={18}/><span>Sarah will stop after the first qualifying quote. Creating this request pre-authorises one purchase order up to {draft.budget?`$${draft.budget}`:'your maximum budget'} only when the exact item, quantity, delivery, net {draft.minimumPaymentDays}+ day payment term, and maximum {draft.maximumDepositPercent}% deposit rules all pass.</span></p>}
