...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className={mobileDetail ? 'mobile-detail-active' : ''}>
      ...
      {page === 'Overview' && <>
        <section className="voice-entry"><div className="voice-orb"><Mic size={27}/></div><div><span className="eyebrow">ONE CONVERSATION. PROCUREMENT HANDLED.</span><h2>“I need 30 kilos of chicken by 8 am, under $350.”</h2><p>Your agent calls authorised suppliers, compares quotes, and gets it sorted.</p></div><button className="primary" onClick={() => setModal(true)}><Mic size={16}/> Start talking</button></section><div className="flow-strip"><span><Mic size={14}/> Tell us what you need</span><ChevronRight size={13}/><span><ShieldCheck size={14}/> ABN-verified suppliers</span><ChevronRight size={13}/><span><Phone size={14}/> Call & compare</span><ChevronRight size={13}/><span><CheckCheck size={14}/> You approve any order</span></div><section className="metrics"><Metric title="Active recoveries" value={String(recoveries.filter(r => r.status !== 'Approved').length).padStart(2,'0')} note={`${recoveries.filter(r => r.status === 'Needs approval').length} waiting for your decision`} icon={<Package size={18}/>}/><Metric title="Recovered this week" value={String(recoveries.filter(r => r.status === 'Approved').length).padStart(2,'0')} note={supabase ? 'Saved in your workspace' : 'From your demo requests'} icon={<CheckCheck size={18}/>}/><Metric title="Time spent on calls" value={supabase ? '—' : '3m 12s'} note="Handled by your voice agent" icon={<Phone size={18}/>}/><Metric title="Below budget" value={supabase ? '—' : '$35'} note="On the recommended recovery" icon={<ArrowDownLeft size={18}/>}/></section>
        ...
        {!seeded ? requestOffers.length ? <SupplierRanking offers={requestOffers} request={request} onTranscript={setTranscript}/> : <div className="empty-state"><span className="empty-icon">{approved ? <CheckCheck/> : <Phone/>}</span><h3>{approved ? 'Recovery approved' : 'Ready for supplier outreach'}</h3><p>{approved ? 'This recovery has already been approved.' : 'Your request is captured. Authorise a verified supplier to start a live quote call.'}</p><span className="demo-badge">{supabase?'Waiting for live quotes':'Demo · No live calls'}</span></div> : <>
...
function TranscriptDialog({offer,onClose}:{offer:Offer;onClose:()=>void}) {return <dialog className="transcript-panel" ref={node=>{if(node&&!node.open)node.showModal()}} onCancel={onClose} aria-label="Call transcript"><button className="secondary" autoFocus onClick={onClose}><ArrowLeft size={16}/> Back to workspace</button><div className="eyebrow">{offer.live?'VERIFIED PROVIDER TRANSCRIPT':'DEMO CALL TRANSCRIPT'}</div><h2>{offer.name}</h2><p className="muted">{offer.minutes} · Completed</p>{offer.live?<><div className="waveform"><span><Check size={14}/> ElevenLabs post-call webhook received</span></div>{offer.transcript?.map((turn,index)=><div className={`speech ${turn.role==='user'?'supplier':''}`} key={index}><span>{turn.role==='user'?offer.name.toUpperCase():'BACKFILL AGENT'}</span><p>{turn.message}</p></div>)}<div className="channel-note"><ShieldCheck size={20}/><p>Webhook signature verified. Quote fields are extracted only from supplier turns and remain reviewable against this transcript.</p></div></>:<><div className="waveform">{Array.from({length:48},(_,i)=><i key={i} style={{height:`${12+((i*17)%35)}px`}}/>)}<span><Check size={14}/> Call complete</span></div><div className="speech"><span>BACKFILL AGENT</span><p>Hi, I’m calling on behalf of Flinders Kitchen. Can you supply 30kg of chicken breast, delivered to Flinders Lane before 8 am tomorrow?</p></div><div className="speech supplier"><span>{offer.name.toUpperCase()}</span><p>We have {offer.quantity} kilos available. The total is {money(offer.price)}, including delivery. We can deliver {offer.delivery.toLowerCase()}.</p></div><div className="speech"><span>BACKFILL AGENT</span><p>Could you offer at least 14 days from invoice, with no deposit? Please confirm any fees and the final total.</p></div><div className="speech supplier"><span>{offer.name.toUpperCase()}</span><p>We can move from {paymentLabel(offer.originalPaymentDays).toLowerCase()} to {paymentLabel(offer.paymentDays).toLowerCase()}. The deposit is {offer.depositPercent}%, with {money(offer.fees)} in added fees. Total payable is {money(offer.price+offer.fees)}.</p></div><div className="speech"><span>BACKFILL AGENT</span><p>Thank you. I’ll pass those details to the owner for review. This is a quote enquiry, so no order is being placed.</p></div><div className="channel-note"><ShieldCheck size={20}/><p>Illustrative transcript for the UI prototype. A real call recording is not connected.</p></div></>}</dialog>}
