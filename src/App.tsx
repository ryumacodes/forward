...
import { approveLocalOffer, loadLocalWorkspace, offerFromLocalCall, repeatLocalOrder, resetLocalWorkspace, saveLocalWorkspace, subscribeLocalCalls, type LocalOrder } from './features/local-demo/store'
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    let cancelled=false
    void loadLocalWorkspace().then(data=>{if(cancelled)return;setRequests(data.requests);setImportedSuppliers(data.suppliers);setLiveOffers(data.offers);setCallQueue(data.callQueue);setLocalOrders(data.orders);setSelected(data.requests[0]?.id??'');setLocalReady(true)}).catch(error=>setLoadError(error instanceof Error?error.message:'Could not open the local demo database.')).finally(()=>{if(!cancelled)setLoading(false)})
    return()=>{cancelled=true}
  },[])
  useEffect(()=>{
    if(!localDemoMode||!localReady)return
    const timer=window.setTimeout(()=>{void loadLocalWorkspace().then(current=>saveLocalWorkspace({requests,suppliers:importedSuppliers,offers:liveOffers,callQueue,discoveries:current.discoveries,orders:localOrders}))},80)
    return()=>window.clearTimeout(timer)
  },[requests,importedSuppliers,liveOffers,callQueue,localOrders,localReady])
  useEffect(()=>{
    if(!localDemoMode)return
    return subscribeLocalCalls(session=>{
      ...
      if(session.kind==='order_confirmation'&&session.status==='completed'){void loadLocalWorkspace().then(data=>{setLocalOrders(data.orders);setRequests(data.requests)});setNotice(`Order ${session.orderId} confirmed by ${session.supplier.name}. Payment is due on delivery; no card was charged.`);return}
  ...
  const refreshWorkspace=async()=>{
    if(localDemoMode){const data=await loadLocalWorkspace();setRequests(data.requests);setImportedSuppliers(data.suppliers);setLiveOffers(data.offers);setCallQueue(data.callQueue);setLocalOrders(data.orders);return}
  ...
  const approveDemoOffer=async(offer:Offer)=>{
    const popup=window.open('about:blank','_blank')
    try{const result=await approveLocalOffer(request,offer,importedSuppliers);if(!result.session)throw new Error('The confirmation session is unavailable.');setRequests(previous=>previous.map(item=>item.id===request.id?{...item,status:'Approved'}:item));await refreshWorkspace();const url=`/?supplier-simulator=1&session=${encodeURIComponent(result.session.id)}`;if(popup)popup.location.href=url;else window.open(url,'_blank');setNotice(`Owner approval recorded for ${offer.name}. Sarah's second simulated call is ringing to confirm ${result.order.id} with payment on delivery.`)}catch(error){popup?.close();throw error}
  }
  const repeatDemoOrder=async(order:LocalOrder)=>{const created=await repeatLocalOrder(order.id);await refreshWorkspace();setSelected(created.id);setPage('Requests');setNotice(`${created.id} created as a repeat of ${order.id}. Review its deadline, then begin supplier sourcing.`)}
  const openConfirmationCall=(order:LocalOrder)=>window.open(`/?supplier-simulator=1&session=${encodeURIComponent(order.confirmationSessionId)}`,'_blank','noopener')
  useEffect(()=>{
  ...
  return <div className="app-shell">
    ...
    <main className={mobileDetail ? 'mobile-detail-active' : ''}>
      ...
      {page === 'Overview' && <>
        ...
        {tab === 'Supplier quotes' && <SupplierRanking offers={requestOffers} request={request} organizationId={organization?.id} onTranscript={setTranscript} onLocalApprove={localDemoMode?approveDemoOffer:undefined}/>}
        {tab === 'Call transcripts' && <div className="quotes-view">{requestOffers.map(o => <CallRow key={o.id} offer={o} onClick={() => setTranscript(o)}/>)}</div>}
        </>}
        </section></div>
      </>}
      {page === 'Requests' && <RequestsPage requests={requests} offers={workspaceOffers} selectedId={selected} live={Boolean(supabase)||localDemoMode} onSelect={setSelected} onCreate={() => setModal(true)} onAddRequest={addRequest} onTranscript={setTranscript} onLocalApprove={localDemoMode?approveDemoOffer:undefined}/>}
      ...
      {page === 'Call activity' && <section className="activity-page"><div className="section-header"><h2>Recent supplier calls</h2><span className="demo-badge">{supabase ? `${workspaceOffers.length} live quote transcripts` : localDemoMode?`${workspaceOffers.length} recorded quote calls`:'3 demo conversations'}</span></div>{workspaceOffers.map(o => <CallRow key={o.id} offer={o} onClick={() => setTranscript(o)}/>)}{workspaceOffers.length===0&&<p className="import-empty">No completed supplier quote calls yet.</p>}<div className="channel-note"><Phone size={20}/><div><strong>A conversation comes first.</strong><p>SMS and email are follow-up channels when a supplier needs written details. No follow-ups sent.</p></div></div></section>}
      {page === 'Call activity' && localDemoMode&&<LocalOrderHistory orders={localOrders} onRepeat={repeatDemoOrder} onOpenCall={openConfirmationCall}/>}
