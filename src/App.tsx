...
import { loadWorkspace, saveProcurementRequest, saveSupplier, type CallQueueItem, type Organization } from './lib/supabase/workspace'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    let cancelled=false
    loadWorkspace().then(data=>{if(cancelled)return;setOrganization(data.organization);setOrganizations(data.organizations);setRequests(data.requests);setImportedSuppliers(data.suppliers);setLiveOffers(data.offers);setCallQueue(data.callQueue);setSelected(data.requests[0]?.id ?? '')}).catch(error=>{if(!cancelled)setLoadError(error.message || 'Could not load your workspace.')}).finally(()=>{if(!cancelled)setLoading(false)})
    return ()=>{cancelled=true}
  },[owner?.id])
  const switchOrganization=async(organizationId:string)=>{
    setLoading(true);setLoadError('')
    try{const data=await loadWorkspace(organizationId);setOrganization(data.organization);setOrganizations(data.organizations);setRequests(data.requests);setImportedSuppliers(data.suppliers);setLiveOffers(data.offers);setCallQueue(data.callQueue);setSelected(data.requests[0]?.id??'')}
  ...
  const refreshWorkspace=async()=>{
    ...
    setRequests(data.requests);setImportedSuppliers(data.suppliers);setLiveOffers(data.offers);setCallQueue(data.callQueue)
  }
  useEffect(()=>{
