...
Deno.serve(async request=>{
  ...
  try{
    ...
    const {action,supplierId}=await request.json() as {action:'verify'|'authorise';supplierId:string}
    if(!['verify','authorise'].includes(action)||!supplierId)return json({error:'Send a valid supplier and action.'},400)
    const {data:supplier,error:supplierError}=await userClient.from('supplier_imports').select('id,name,abn,authorised').eq('id',supplierId).single()
    if(supplierError||!supplier)return json({error:'Supplier was not found in this owner workspace.'},404)
    const serviceClient=createClient(supabaseUrl,serviceKey,{auth:{persistSession:false,autoRefreshToken:false}})
    if(action==='authorise'){
      const {data:evidence,error:evidenceError}=await serviceClient.from('supplier_verifications').select('active,legal_name,gst_registered,name_matched,contact_confirmed,checked_at').eq('supplier_id',supplier.id).eq('owner_id',user.id).single()
      ...
      const {error:confirmError}=await serviceClient.from('supplier_verifications').update({contact_confirmed:true}).eq('supplier_id',supplier.id).eq('owner_id',user.id)
      if(confirmError)throw confirmError
      const {error:authoriseError}=await serviceClient.from('supplier_imports').update({authorised:true,authorised_at:new Date().toISOString()}).eq('id',supplier.id).eq('owner_id',user.id)
    ...
    endpoint.searchParams.set('abn',supplier.abn);endpoint.searchParams.set('callback','backfill');endpoint.searchParams.set('guid',guid)
    ...
    const {error:saveError}=await serviceClient.from('supplier_verifications').upsert({supplier_id:supplier.id,owner_id:user.id,active:verification.active,legal_name:verification.legalName,gst_registered:verification.gstRegistered,name_matched:verification.nameMatched,contact_confirmed:false,source:'ABR',checked_at:verification.checkedAt})
    if(saveError)throw saveError
    if(supplier.authorised){await serviceClient.from('supplier_imports').update({authorised:false,authorised_at:null}).eq('id',supplier.id).eq('owner_id',user.id)}
