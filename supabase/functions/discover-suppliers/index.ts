...
Deno.serve(async request=>{
  ...
  try{
    ...
    const {product,location,profileId}=await request.json() as {product:string;location:string;profileId:string}
    if(!product?.trim()||product.length>200||!location?.trim()||location.length>200||!['hospitality','construction','general'].includes(profileId))return json({error:'Send a valid product, location, and buying profile.'},400)
    ...
    const suppliers=await Promise.all(usable.map(async(item,index)=>{
      ...
      const {data,error}=await serviceClient.from('supplier_evidence').upsert({owner_id:user.id,search_query:query,supplier_name:item.name,website_url:item.websiteUrl,source_url:source.url,source_title:source.title,content_excerpt:item.excerpt,extracted_facts:facts,embedding_model:embeddingModel,embedding,source_hash:sourceHash,checked_at:checkedAt},{onConflict:'owner_id,source_hash'}).select('id').single()
      if(error)throw error
      return {id:data.id,name:item.name,websiteUrl:item.websiteUrl,locality:item.locality,summary:item.summary,products:item.products,phone:item.phone,email:item.email,confidence:item.confidence,semanticScore,sources:item.sources,evidenceCheckedAt:checkedAt,mode:'live' as const}
    }))
    suppliers.sort((a,b)=>b.semanticScore-a.semanticScore||b.confidence-a.confidence||a.name.localeCompare(b.name))
...
async function scrapePublicPage(value:string){
  ...
  const response=await fetch(url,{redirect:'manual',headers:{Accept:'text/html,text/plain;q=0.9','User-Agent':'BackfillSupplierEvidence/1.0'}})
