...
test('Supabase schema isolates owners and protects backend-only evidence', async()=>{
 ...
 try {
  ...
  for (const migration of ['20260912054141_procurement_workspace.sql','20260912070155_procurement_policy_and_discovery.sql']) await db.exec(await readFile(new URL(`../supabase/migrations/${migration}`,import.meta.url),'utf8'))
