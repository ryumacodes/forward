...
test('Supabase schema isolates owners and protects backend-only evidence', async()=>{
 ...
 try {
  ...
  await expect(db.query("update public.recovery_requests set status='Approved' returning id")).rejects.toThrow()
